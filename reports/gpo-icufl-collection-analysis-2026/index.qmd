...
Rather than attempt to write a script (or do anything else complicated that it coulg go wrong), we used search and replace commands in the text editor (first `/[0-9]+-mets.xml/$0 local-mets\/$0/` then `/^/cp -n /`) to turn each line into a script that would copy each METS file into a `local-mets` folder:
...
Some general notes:
 ...
 - .ISO images can be directly mounted in Windows, or opened with 7-Zip
 - .IMG floppy images can be opened with [FluxFox](https://dbalsom.github.io/fluxfox/) and in this case the files can generally be extracted that way as these are 'simple' PC floppy images.
 - Many of the disks have quite good build-in documentation e.g [README.TXT](https://webapp1.dlib.indiana.edu/virtual_disk_library/index.cgi/4288667/FID1811/README.TXT)
     - "Note that all SAS data sets (.SSD) are in version 6.03,  and that all WordPerfect files (.WPD) are version 6.1 for Windows."
 - Looking for '.VOB' files in the Explorer (above) showed that some items are DVDs and would be accessible with e.g. VLC.
...
If you require a SAS licence, I can put you in touch with the ales team here at SAS. Please, let me know if there's anything else I can assist you with.
...
Note that cross searching for format registries for [.sd2](https://www.digipres.org/workbench/formats/lookup#*.sd2) indicated this may be 'Sound Designer II' files or SAS statistics software files ([WikiData reference only](https://www.wikidata.org/wiki/Q47483338)). At the time of writing, none of the existing sources capture that this file extension corresponds to SAS 6.
...
So these are similar to the prior case, but here no DATA alternative has been supplied. So while the original files can be retained, access will remain challenging.
...
However, on closer inspection the PDF are found to be interlinked, using relative hyperlinks to move between the different PDFs. If the PDF's are unpacked into local files, this still works fine. However, this interconnection would be lost if the PDFs were ingesting in a preservation system as separate items. So even though these are 'just' PDFs, preserving the intended publication required the set of files to be kept together in the supplied heirarchy.
...
2. **Is this collection of disk images self-consistent? Does it appear to be valid?**
   - Largely self-consistent, but some items lack metadata, and the XML does not appear to correctly follow the intended METS schema.
3. **Is this what these kind of captures 'typically' look like? What have others done with large collections of disk images like this?**
...
4. **What files and formats are inside the disk images? Could the files be kept as plain files rather than disk images? Can we scan and classify files and formats as _'easy to access'/'safe to ignore'/'requires investigation'_?**
   ...
   - But this is not universal, e.g. software disks or DVD Video.
   - It was not possible to classify files as suggested. The investigation indicates that it might be possible to classify sets of related file extensions in this way, but this would require a significant amount of follow-up work.
5. **What might access look like? Does emulation play a role in this?**
   - The GPO directly supporting access to this very varied collection would likely be challenging.
   - However, the GPO could make detailed information about the contents available, so as to encourage discovery and use, and support researchers who have the time and resources to restore access to these files. 
   ...
   - The results of working with researchers could be folded back into the collection as appropriate. e.g. if the researcher manages to migrate `.SD2` files to `.CSV` or similar.


[^1]: This involved making sure the Parquet 'row groups' were large and hence not too numerous, as the metadata of each is loaded sequentially. Get in touch if you want more information on this!